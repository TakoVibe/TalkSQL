...
export function ThemeToggle() {
  ...
  return <select ref={ref} aria-label="Theme" defaultValue="system" onChange={e => { const value = e.target.value as Theme; localStorage.setItem("talksql.theme", value); apply(value); }} className="min-h-11 cursor-pointer rounded-xl border border-[var(--border)] bg-[var(--surface)] px-2 py-2 text-xs text-[var(--ink-muted)]"><option value="system">System</option><option value="light">Light</option><option value="dark">Dark</option></select>;
}
